...
public:
    ...
    float calc_median_coverage() const {
        if (_coverages.empty()) {
            std::cerr << "Error: _coverages is empty, cannot compute median." << std::endl;
            return 0.0f; // или выбросить исключение, если нужно
        }
        ...
        return size % 2 == 0
            ? (sorted_coverages[size / 2 - 1] + sorted_coverages[size / 2]) / 2.0f
